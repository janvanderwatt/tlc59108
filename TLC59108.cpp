...
TLC59108::TLC59108(const byte i2c_address) : i2c(&Wire), addr(i2c_address) {
}
...
uint8_t TLC59108::init(const uint8_t hwResetPin) {
    ...
    return setRegister(REGISTER::MODE1::ADDR, 0);
}
...
uint8_t TLC59108::setLedOutputMode() {
    uint8_t retVal = setRegister(REGISTER::LEDOUT0::ADDR, (TLC59108::outputMode)&0xFF);
    retVal &= setRegister(REGISTER::LEDOUT1::ADDR, (TLC59108::outputMode >> 8) & 0xFF);
    return retVal;
}
...
uint8_t TLC59108::setLedOutputMode(const uint8_t outputMode) {
    ...
    TLC59108::outputMode = (outputMode << 6) | (outputMode << 4) | (outputMode << 2) | outputMode;
    return setLedOutputMode();
}

uint8_t TLC59108::setLedOutputMode(uint8_t pwmChannel, const uint8_t outputMode) {
    if (outputMode & 0xfc)
        return ERROR::EINVAL;

    pwmChannel *= 2;                                      // There are 2 bits per channel, so shift twice as many bits
    TLC59108::outputMode &= (3 << (pwmChannel)) ^ 0xFFFF; // clear the bits for this channel
    TLC59108::outputMode |= outputMode << pwmChannel;     // replace the bits for this channel
    return 0;
}
...
uint16_t TLC59108::outputMode = 0;