...
public:
	...
	// sets the specific channel to the output mode listed in REGISTER::LED_MODE, but does not update the peripheral
	uint8_t setLedOutputMode(uint8_t pwmChannel, const uint8_t outputMode);

    // sets channel to the output modes held in listed in TLC59108::outputMode
    uint8_t setLedOutputMode();
